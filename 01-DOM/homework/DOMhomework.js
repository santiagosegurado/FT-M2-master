...
createdBy.textContent += ' Santiago Segurado';
...
function buildToDo(todo, index) {
  ...
  // 6)
  toDoText.classList = todo.complete ? 'completeText' : '';
...
function buildToDos(toDos) {
  ...
  toDos.map((todo,index)=>{
    newArray.push(buildToDo(todo,index));
  })
...
function displayToDos() {
  ...
  // 3)
  let newToDos = buildToDos(toDoItems);
...
function addToDo() {
  // Tu código acá:
  // 1)
  let inputodo = document.querySelector('#toDoInput');
  let newTodos = new ToDo(inputodo.value);
